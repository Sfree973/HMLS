...
#GAT 
        neighbour_cnt_list = [seen_nodes.shape[0] for seen_nodes in seen_nodes_embeddings_list]
        split_point_list = [0] + np.cumsum(neighbour_cnt_list).tolist()

        now_projection =  nn.Linear(seen_nodes_dim * 2, 1)
        neighbour_projection = nn.Linear(torch.cat(neighbour_embeddings_list, dim=0))
        now_projection = [projection.expand(neighbour_cnt, seen_nodes_dim * 2) for (projection, neighbour_cnt) in zip(now_projection, neighbour_cnt_list)]
        now_projection = torch.cat(now_projection, dim=0)
        gat_projection = torch.cat([now_projection, neighbour_projection], dim=1)
        gat_projection =  nn.Linear(gat_projection_dim * 2, 1)
        gat_projection = self.leakyrelu(gat_projection)
        gat_projection = gat_projection.squeeze(1)
        gat_projection_list = [gat_projection[split_point_list[i] : split_point_list[i + 1]] for i in range(len(neighbour_cnt_list))]
        gat_projection_list = [self.softmax(gat_projection).unsqueeze(0) for gat_projection in gat_projection_list] # Attention weight of each neighbour node
        gat_projection_list = [torch.mm(gat_projection, neighbour_embeddings) for (gat_projection, neighbour_embeddings) in zip(gat_projection_list, neighbour_embeddings_list)] # Hidden vector representation of the current node
        gat_projection = torch.cat(gat_projection_list, dim=0)

        # Representation of the current state (current node and other observations)
        Q = kg.get_relation_embeddings(q)
        H = self.path[-1][0][-1, :, :]
        if self.relation_only:
            X = torch.cat([H, Q,gat_projection], dim=-1)
        elif self.relation_only_in_path:
            E_s = kg.get_entity_embeddings(e_s)
            E = kg.get_entity_embeddings(e)
            X = torch.cat([E, H, E_s, Q,gat_projection], dim=-1)
        else:
            E = kg.get_entity_embeddings(e)
            X = torch.cat([E, H, Q,gat_projection], dim=-1)
