...
from src.HHS import LFramework
import src.HHML.graph_search.beam_search as search
...
class PolicyGradient(LFramework):
    ...
    def meta_loss(self, mini_batch, mini_batch_valid):
        ...
        loss, prec = self.loss(mini_batch)
        # print("model_loss:",loss['model_loss'])
        params_mdl = self.mdl.update_params(
            loss['model_loss'], step_size=self.fast_lr, first_order=False)
        params_kg = self.kg.update_params(
            loss['model_loss'], step_size=self.fast_lr, first_order=False)
        old_params_mdl = parameters_to_vector(self.mdl.parameters())
        old_params_kg = parameters_to_vector(
            filter(lambda p: p.requires_grad, self.kg.parameters()))
        vector_to_parameters(params_mdl, self.mdl.parameters())
        vector_to_parameters(params_kg, filter(
            lambda p: p.requires_grad, self.kg.parameters()))
        loss1, _ = self.loss(mini_batch_valid)
        vector_to_parameters(old_params_mdl, self.mdl.parameters())
        vector_to_parameters(old_params_kg, filter(
            lambda p: p.requires_grad, self.kg.parameters()))
        return loss1, prec
    ...
    def loss(self, mini_batch):
        ...
        # print("-----------")
        # print("e1", r, self.num_rollout_steps)
        ...
        # print("pred_e2", output['pred_e2'].shape)
        # print("log_action_probs", output['log_action_probs'][0].shape,output['log_action_probs'][0])
        # print("action_entropy", output['action_entropy'][0].shape,output['action_entropy'][0])
        # print("path_trace", len(output['path_trace'][0]),output['path_trace'][0])

        # Compute hierarchical reward
	entity_reward = path_hop/n_fun(path_trace)
	if len(path_trace) != 0:
		...
		path_reward = -np.mean(cos_sim)
        final_reward = self.reward_fun(e1, r, e2, pred_e2, path_trace)
        if self.baseline != 'n/a':
            final_reward = stablize_reward(final_reward)
        intrinsic_reward = entity_reward*min(path_reward,1) 
        extrinsic_reward = [0] * self.num_rollout_steps
        cum_discounted_rewards = extrinsic_reward  + self.delta*intrinsic_reward 
        ...
        entropy = torch.cat([x.unsqueeze(1)
                             for x in action_entropy], dim=1).mean(dim=1)
        ...
        return loss_dict, torch.sum(final_reward)
    ...
    def sample_action(self, db_outcomes, inv_offset=None):
        ...
        def apply_action_dropout_mask(action_dist, action_mask):
            if self.action_dropout_rate > 0:
                ...
                sample_action_dist = \
                    action_dist * action_keep_mask + ops.EPSILON * \
            ...
            for i in range(len(e1)):
                for j in range(output_beam_size):
                    ...
                    for k in range(len(search_traces)):
                        search_trace.append(
                            (int(search_traces[k][0][ind]), int(search_traces[k][1][ind])))
                    print('beam {}: score = {} \n<PATH> {}'.format(
    ...
    def record_path_trace(self, path_trace):
        ...
        path_trace_mat = torch.cat(
            flattened_path_trace).reshape(-1, path_length)
