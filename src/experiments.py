...
from src.HHML.graph_search.pn import GraphSearchPolicy
from src.HHML.graph_search.pg import PolicyGradient
from src.HHML.graph_search.rs_pg import RewardShapingPolicyGradient
...
def train(lf):
    ...
    if args.few_shot or args.adaptation:
        normal_train_data, few_train_data = data_utils.load_triples(train_path, entity_index_path, relation_index_path, group_examples_by_query=args.group_examples_by_query, add_reverse_relations=args.add_reversed_training_edges, few_shot=True, lf=lf)
    else:
        train_data = data_utils.load_triples(train_path, entity_index_path, relation_index_path, group_examples_by_query=args.group_examples_by_query, add_reverse_relations=args.add_reversed_training_edges)
    if 'NELL' in args.data_dir:
        ...
        seen_entities = set()
    if args.few_shot or args.adaptation:
        normal_dev_data, few_dev_data = data_utils.load_triples(dev_path, entity_index_path, relation_index_path, seen_entities=seen_entities, few_shot=True, lf=lf)
    else:
    ...
    if args.adaptation:
        for x in few_train_data:
            ...
            lf.run_train(few_train_data[x], few_dev_data, adaptation=True, adaptation_relation=x)
    elif args.few_shot:
        if args.checkpoint_path is not None:
    ...
    if args.compute_map:
        ...
        if args.few_shot:
            ...
            _, dev_data = data_utils.load_triples(
                dev_path, entity_index_path, relation_index_path, seen_entities=seen_entities, verbose=False, few_shot=True, lf=lf)
            ...
            num = 0; hits_10 = 0.0; hits_1 = 0.0; hits_3 = 0.0; hits_5 = 0.0; mrr = 0.0
            for x in test_data:
                lf.load_checkpoint(args.checkpoint_path.replace('[relation]', str(x)))
            ...
            pred_scores = lf.forward(test_data, verbose=False)
            test_metrics = src.eval.hits_and_ranks(test_data, pred_scores, lf.kg.all_objects, verbose=True, output=True, kg=lf.kg, model_name=args.model)
            eval_metrics['test']['hits_at_1'] = test_metrics[0]
            eval_metrics['test']['hits_at_3'] = test_metrics[1]
            eval_metrics['test']['hits_at_5'] = test_metrics[2]
            eval_metrics['test']['hits_at_10'] = test_metrics[3]
